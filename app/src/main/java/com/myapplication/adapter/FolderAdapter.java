...
public class FolderAdapter extends BaseAdapter {
    ...
    public FolderAdapter(Context context) {
        ...
        mImageSize = mContext.getResources().getDimensionPixelOffset(R.dimen.space_70);
    }
    ...
    public View getView(int i, View view, ViewGroup viewGroup) {
        ...
        if (holder != null) {
            if (i == 0) {
                holder.name.setText(mContext.getResources().getString(R.string.select_picture_album_all));
                holder.size.setText("(" + getTotalImageSize() + "张)");
                if (mFolders.size() > 0) {
                    FolderInfo f = mFolders.get(0);
                    Glide.with(mContext)
                            .load(new File(f.cover.path))
                            .placeholder(R.mipmap.camerasdk_pic_loading)
